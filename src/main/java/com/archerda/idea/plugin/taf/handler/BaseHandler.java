...
public abstract class BaseHandler implements Handler {
        ...
        for (int i = 0; i < lineContents.length; i++) {
            if (StringUtils.isNotBlank(lineContents[i])) {
                if (lineContents[i].contains("id=\"" + matchContent + "\"")
                    ...
                    // 对于 Feign 方法实现的跳转来说，匹配的行的格式应该是 "listBaseUser"
                    // 对于 Feign 方法定义的跳转来说，匹配的行的格式应该是 "listBaseUser"、"/listBaseUser"
                    // 对于 Mapper 接口中的方法来说，匹配的行的格式应该是 listBaseUser(
                    return i;
                }
            }
        }
     ...
     * @return 是/否成功定位
     */
    public final boolean doLocate(Project project, String matchContent, PsiFile file) {
        int lineNumber = getLineNumber(file, matchContent);
        if (lineNumber == 0) {
            return false;
        }
        ...
        // 移动到目标行
        caretModel.moveToLogicalPosition(new LogicalPosition(lineNumber, logicalPosition.column));
        // 将目标行滚动到屏幕中央
